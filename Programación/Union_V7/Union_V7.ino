...
int tdsSensorPin = 34; // Usar un pin ADC1 (ej. 32, 33, 34, 35, 36, 39)
...
float leerCalidadAgua() {
  ...
  if (adc_valor == 0 || adc_valor >= 4095) {
    // Si la lectura es 0 o el máximo, es probable que haya un fallo (corto/abierto)
    return -1.0; 
  }
  float voltaje = adc_valor * (VREF / 4095.0); // Convertir lectura a voltaje
  // Fórmula de conversión de voltaje a TDS (PPM).
  // Esta fórmula es genérica y puede necesitar calibración para tu sensor específico.
...
String paginaHTML() {
  ...
  <head>
    ...
    <style>
      ...
      /* --- NUEVO: Estilos para el indicador de calidad --- */
      .quality-indicator {
        text-align: center;
      }
      #qualityLevel {
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="card">
        ...
        <div class="sensor-item">
          <span><i class="fas fa-vial"></i> Calidad del Agua:</span>
          <span id="calidadAgua">---</span>
        </div>
      </div>

      <!-- --- NUEVO: Tarjeta para el Nivel de Calidad --- -->
      <div class="card">
      ...
      function actualizar() {
        fetch('/data').then(res => res.json()).then(data => {
          ...
          if (data.calidadAgua < 0) {
            ...
            errorMessages.push('¡Fallo en el sensor de calidad!');
          } else {
            calidadSpan.innerHTML = data.calidadAgua.toFixed(0) + " PPM";
          }

          // --- NUEVO: Lógica para el indicador de calidad de agua ---
          const qualityLevel = document.getElementById('qualityLevel');
          const qualityText = document.getElementById('qualityText');
          const ppm = data.calidadAgua;
          let levelText = '---';
          let levelColor = '#343a40'; // Color por defecto (negro)

          if (ppm < 0) {
            levelText = 'ERROR';
            levelColor = '#ffc107';
            qualityText.textContent = 'Fallo en sensor';
          } else if (ppm <= 50) {
            levelText = 'Ideal para Beber';
            levelColor = '#34C7D9';
          } else if (ppm <= 100) {
            levelText = 'Filtros de Carbón';
            levelColor = '#5B9BD5';
          } else if (ppm <= 200) {
            levelText = 'Agua Dura';
            levelColor = '#7030A0';
          } else if (ppm <= 300) {
            levelText = 'Marginalmente Aceptable';
            levelColor = '#92D050';
          } else if (ppm <= 400) {
          ...
          qualityText.textContent = `(Lectura: ${ppm.toFixed(0)} PPM)`;
